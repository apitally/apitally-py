...
**Status:** Fixed. `ApitallyMetricReader.collect` calls `drop_empty_histogram_aggregations` before collecting, which drops every histogram aggregation that recorded nothing since the previous cycle. Aggregations holding data are never dropped, so a measurement that lands during collection exports on the next cycle. Covered by `test_collect_appends_delta_payloads_to_spool`.
...
**Status:** Fixed. `metrics.setup` no longer touches `SystemMetricsInstrumentor`. The two process metrics are observed directly via psutil on the `apitally` meter with the same names, kinds and units as before (`process.cpu.utilization` observable gauge, unit `1`; `process.memory.usage` observable up-down counter, unit `By`), and the `psutil.Process` is created in `setup` so fork re-activation observes the child. The `opentelemetry-instrumentation-system-metrics` dependency is replaced by `psutil`. Re-verified in both orders with the instrumentor installed alongside: the user's 30 metrics stay intact with no callback errors, and a later user `instrument()` is no longer refused. The two metrics now report under the `apitally` instrumentation scope; the ingest matches process metrics by name across all scopes (spec section 7.2), so no server change is needed.
...
**Status:** Rejected. The scan only runs on the drop branch of a user-configured `sample_on_response`. Re-measured at 13 us per drop with 400 in-flight spans and 71 us with 2,000, which is around one percent of a core at realistic per-process loads; the quoted tens of percent needs thousands of in-flight spans and thousands of drops per second in one Python process. The re-insert leak is fixed separately: the drop loop only re-marks entries still present in `spans`, which narrows the window from the whole scan to the check itself. A per-request descendant index would add a fifth piece of per-request state to keep consistent across start, end, drop and fork reset for a marginal gain. Revisit if response sampling is used at very high concurrency.
