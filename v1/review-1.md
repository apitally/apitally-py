...
- **R6:** §8's "first call wins" silently strands a differing re-call's settings; no warning mechanism for ignored settings is stated. (adversarial) [REJECTED]
...
- **G12 (from finding 2):** Naming: §5/§16 now say `ApitallyPlugin`; litestar.md's example uses a lowercase factory `apitally_litestar_plugin(...)` — align litestar.md to the winner. Also worth a sentence: what §8 re-call semantics mean for a second `ApitallyPlugin` construction. → DECIDED (grilling): `ApitallyPlugin` wins; litestar.md aligned, and its `on_app_init` goes through the §8 config singleton so the re-call semantics match the other frameworks.
