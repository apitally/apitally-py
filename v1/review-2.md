...
- What does `ApitallyPlugin` do when the user's Litestar plugin list already contains Litestar's stock `OpenTelemetryPlugin`? Two OTel configs would otherwise yield nested SERVER/INTERNAL span pairs per request; §4's detection list omits Litestar. (adversarial) → DECIDED (grilling 5, 2026-07-04): verified against Litestar 2.24 — the stock OTel middleware is applied at app level from the plugin registry (fetched by class name in `_create_asgi_handler`), exactly one per app, so nested SERVER spans are structurally impossible; regular middleware runs route-level inside it. `ApitallyPlugin.on_app_init` detects user instrumentation as a stock plugin in `app_config.plugins` OR an OTel `DefineMiddleware` in `app_config.middleware` (legacy pre-2.22 pattern, hoisted by `_patch_opentelemetry_middleware`) → skip ours; otherwise installs by appending `OpenTelemetryPlugin(our_config)` to `app_config.plugins`, avoiding the hoist workaround's last-one-wins pop. Applied to §4 and litestar.md (new mechanics section).
- For the §2 cooperative attribute-limit warning: does the inspection also cover `OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT` set in the environment when the user's provider was constructed without explicit SpanLimits, or only the provider's effective `_span_limits`? (adversarial) → ANSWERED BY RESEARCH (grilling 5, 2026-07-04): covered — `TracerProvider.__init__` defaults to `span_limits or SpanLimits()`, and `SpanLimits()` resolves `OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT` / `OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT` at construction, so the effective `_span_limits` the §2 inspection reads already reflects env-var limits. Clarifying parenthetical added to §2; no design change.
