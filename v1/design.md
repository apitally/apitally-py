...
- On `on_end`: if `keep` is true, forward to the wrapped batch processor; otherwise drop. Pop the span_id from the in-flight map. For a kept SERVER span, the response-stage sampling decision runs first when a `sample_on_response` callback (§5) is configured: its keep probability is tested against the same trace ID, so both stages agree per trace and the overall capture probability is the minimum of the two rates, not their product; `None` abstains, leaving the request-stage decision standing. The SERVER span is the keystone: without it no request row can materialize server-side, and descendants or logs that already streamed out are discarded as unclaimed orphans at ingest (out-of-order batch arrival forces the ingest to handle those regardless; the GC rule is specified upstream in the spec). Combined with `set_request_attribute(...)`, this lets users sample on their own business attributes. Callback errors follow §9: warn, count as keep.
...
| `sample_on_response` | `Callable[[ReadableSpan], float \| bool \| None]` | `None` | Same signature, decided at SERVER span end (§3); `None` leaves the request-stage decision standing. Cannot unsend mid-request telemetry: already-streamed descendants and logs are discarded at ingest but count toward quota — `sample_rate` / `sample_on_request` are the volume and quota lever, this is the keep-the-interesting-ones refinement. Filter on `http.response.status_code`, `apitally.consumer.*`, custom attributes. |
