...
- OPTIONS and excluded requests are decided at the same point: a SERVER span that would otherwise enter the map as kept instead enters as `(False, None)` when its `http.request.method` is `OPTIONS`, or its `url.path` / `user_agent.original` (old-semconv fallbacks included) matches an exclusion pattern — defaults per spec § 6.8, user path patterns added on top. The stock instrumentors pass these as span-creation attributes, so they are readable via `span.attributes` inside `on_start`; via the shared map this also drops the request's logs (§10). Histogram recording in the glue (§4) is independent: excluded requests are still counted (spec § 6.8), while OPTIONS and unmatched-route requests are not (spec § 7.1). Requests that survive exclusion then take the request-stage sampling decision at the same point: a user-supplied `sample_on_request` callback (§5) — receiving the SERVER span and returning a keep probability — resolves an effective rate (`bool` maps to `1.0`/`0.0`, `None` falls back to `sample_rate`), and the span's trace ID is tested against it using the `TraceIdRatioBased` bound convention (keep iff the low 64 bits fall under `round(rate * 2**64)`), deterministic per trace so services sampling at the same rate capture the same traces. A sampled-out request enters the same `(False, None)` entry — nothing about the request is ever transmitted. The processor also records the outcome in a request-scoped keep flag, which the §6 transports read to skip capture work (body buffering, header/body attribute writes, mask callbacks) for excluded and sampled-out requests alike. Exception: the Flask request-body buffer at WSGI entry runs before the SERVER span exists (§6), so it cannot read the flag and always runs; the gated attribute write discards it.
- On `on_end`: pop the span_id from the in-flight map; a not-kept span is dropped. A kept descendant of a pending request is buffered in the processor, keyed by its SERVER span id, rather than forwarded — the uniform export model: a request's telemetry is exported when the request completes, for every request in every mode. At the SERVER span's end the response-stage sampling decision runs (trivially keep without a `sample_on_response` callback, §5): the callback's keep probability is tested against the same trace ID, so both stages agree per trace and the overall capture probability is the minimum of the two rates, not their product; `None` abstains, leaving the request-stage decision standing. Keep flushes the buffered descendants (redacted at flush, §6) followed by the SERVER span, and the same decision flushes or discards the request's buffered logs (§10); drop discards the buffers and flips the request's still-in-flight entries to dropped, so late telemetry drops locally — a response-stage drop exports nothing and consumes no quota. Buffers are bounded per request (1,000 spans and 1,000 log records, Sentry parity); at the cap excess items are dropped, so a kept request exports at most the caps. Wrapper `shutdown()` discards pending buffers — an in-flight request's SERVER span can never export after shutdown, so its held telemetry is unreachable by construction. The SERVER span remains the keystone: without it no request row can materialize server-side; spec §6.5's orphan rule stays as ingest robustness for out-of-order batch arrival. Combined with `set_request_attribute(...)`, this lets users sample on their own business attributes. Callback errors follow §9: warn, count as keep.
...
| `sample_rate` | `float` | `1.0` | Fraction of non-excluded requests captured as traces with their logs, range `[0, 1]`, decided deterministically from the trace ID at span start (§3). Metrics always count every request. Invalid values warn once and capture everything (§9). |
| `sample_on_request` | `Callable[[ReadableSpan], float \| bool \| None]` | `None` | Per-request keep probability refining `sample_rate`, decided at span start (§3): `float` in `[0, 1]`, `bool` (`True` = always capture, `False` = never), `None` = fall back to `sample_rate`. Guarantee: nothing about a sampled-out request is transmitted, and capture work is skipped (§3, §6; on Flask the request-body buffer runs regardless, since the decision postdates it). Filter on semconv attributes (`url.path`, `http.request.method`, `user_agent.original`, ...). |
| `sample_on_response` | `Callable[[ReadableSpan], float \| bool \| None]` | `None` | Same signature, decided at SERVER span end (§3); `None` leaves the request-stage decision standing. Quota-safe: the request's spans and logs are held in memory until this decision (§3), so a drop exports nothing and consumes no quota, and a kept request exports at most the per-request buffer caps. Capture work still runs before this decision — `sample_rate` / `sample_on_request` remain the lever for skipping that overhead. Filter on `http.response.status_code`, `apitally.consumer.*`, custom attributes. |
...
A second ContextVar records the keep decision for the current request (set alongside the span handle in `on_start`); the §6 transports read it to skip capture work for excluded and sampled-out requests (except the Flask request-body buffer, which precedes the decision — §6). In cooperative mode this means such requests no longer carry Apitally-written header/body attributes on spans the user's own pipeline exports.
...
- `exclude_callback(request, response)` — replaced by `sample_on_request` / `sample_on_response`, which operate on the SERVER span's OTel attributes instead of bespoke request/response dicts and return a keep probability instead of an exclude boolean (polarity flip: `True` / `1.0` means capture).
...
**Flask span writes are hook-timed, not middleware-timed.** The Flask instrumentor starts the SERVER span in `before_request` and ends it in `teardown_request`, which Flask runs inside its own `wsgi_app` `finally` — before the response iterable reaches any WSGI middleware, inner or outer. No WSGI layer can set response attributes while the span is alive. The Flask adapter therefore writes at two Flask-side points instead: request body and response headers at our wrapped `start_response` (fires inside `wsgi_app` while the span records, with wire-final headers), and the response body in an `after_request` hook (runs before teardown), reading `response.get_data()` when `direct_passthrough` is false. Streaming/direct-passthrough Flask responses are not captured — one documented limitation, no machinery. The WSGI middleware's role on Flask is transport only: `wsgi.input` buffering per the Content-Length gate, the wrapped `start_response`, and duration/size accounting on the response iterable. The §3 keep flag is unreadable at buffer time — the span starts later, in `before_request` — so request-body buffering runs for excluded and sampled-out requests too and the gated write at `start_response` discards the buffer. Buffering in a `before_request` hook instead is not an option: reading the body there exhausts `request.stream` for the view (the NDJSON streaming pattern), whereas the WSGI-entry `wsgi.input` replacement is invisible to the app.
...
| Full request capture (`sample_rate=1.0`; sampling is opt-in) | ✓ |
