...
| `sample_on_request` | `Callable[[ReadableSpan], float \| bool \| None]` | `None` | Per-request keep probability refining `sample_rate`, decided at span start (§3): `float` in `[0, 1]`, `bool` (`True` = always capture, `False` = never), `None` = fall back to `sample_rate`. Guarantee: nothing about a sampled-out request is transmitted, and capture work is skipped (§3, §6). Filter on semconv attributes (`url.path`, `http.request.method`, `user_agent.original`, ...). |
...
A second ContextVar records the keep decision for the current request (set alongside the span handle in `on_start`); the §6 transports read it to skip capture work for excluded and sampled-out requests. In cooperative mode this means such requests no longer carry Apitally-written header/body attributes on spans the user's own pipeline exports.
